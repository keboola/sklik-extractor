    ...
    public function run(Config $config, ?int $globalLimit = null): void
    {
        ...
        foreach ($this->api->getAccounts() as $account) {
            ...
            foreach ($config->getReports() as $report) {
                ...
                // Create report
                $this->logger->info(sprintf('Creating report for resource "%s".', $report['resource']));
                ...
                $this->logger->info(sprintf('Created report for resource "%s".', $report['resource']));
                ...
                if ($offset || $lastRecord) {
                    $this->logger->info(sprintf('Reading records <%d;%d> from resource "%s" report.', $offset, $lastRecord, $report['resource']));
                } else {
                    $this->logger->info(sprintf('Reading all records from resource "%s" report.', $report['resource']));
                }
                ...
                $this->logger->info(sprintf('Records <%d;%d> have been read from resource "%s" report.', $start, $offset, $report['resource']));
            }
        }
    }
}

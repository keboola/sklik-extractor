...
class Extractor
{
    ...
    public function run(Config $config, ?int $globalLimit = null): void
    {
        ...
        if (isset($report['allowedUserIDs'])) {
            $allowedUserIds = (array) $report['allowedUserIDs'];
            $this->logger->info(sprintf(
                'Report for resource "%s" is limited to following user IDs: %s',
                $report['resource'],
                join(', ', $allowedUserIds)
            ));
        }
        ...
        foreach ($this->api->getAccounts() as $account) {
            ...
            if ($allowedUserIds && !in_array($account['userId'], $allowedUserIds, true)) {
                continue;
            }

            $userId = sprintf('#%s', $account['userId']);

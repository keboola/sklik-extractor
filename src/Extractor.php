            ...
            foreach ($config->getReports() as $report) {
                ...
                if ($offset || $lastRecord) {
                    $this->logger->info(sprintf(
                        'Reading records <%d;%d> from resource "%s" report.',
                        $offset,
                        $lastRecord,
                        $report['resource']
                    ));
                    ...
                    $this->logger->info(sprintf(
                        'Reading all records from resource "%s" report.',
                        $report['resource']
                    ));
                }
                ...
                while (true) {
                    ...
                    try {
                        ...
                        $this->logger->error(sprintf('Error when reading %d. batch <%d;%d>: %s', $batch, $offset, $offset + $limit, $e->getMessage()));
                        throw $e;
                    }
                ...
                $this->logger->info(sprintf(
                    'Records <%d;%d> have been read from resource "%s" report.',
                    $start,
                    $offset,
                    $report['resource']
                ));

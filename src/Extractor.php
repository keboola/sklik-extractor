        ...
        foreach ($this->api->getAccounts() as $account) {
            ...
            foreach ($config->getReports() as $report) {
                ...
                if ($offset || $lastRecord) {
                    $this->logger->info(sprintf(
                        'Reading records <%d;%d> from resource "%s" report for account "%s".',
                        $offset,
                        $lastRecord,
                        $report['resource'],
                        $username
                    ...
                    $this->logger->info(sprintf(
                        'Reading all records from resource "%s" report for account "%s".',
                        $report['resource'],
                        $username
                    ));
                }
                ...
                $this->logger->info(sprintf(
                    'Records <%d;%d> have been read from resource "%s" report for account "%s".',
                    $start,
                    $offset,
                    $report['resource'],
                    $username

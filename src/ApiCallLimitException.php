...
class ApiCallLimitException extends Exception
{
    public function getWaitingTimeInSeconds(): int
    {
        ...
        // Defensive fallback: some rate-limit responses report a non-positive wait time
        // (e.g. "Has to wait -2[s].") which the strict pattern above rejects, previously
        // crashing the whole job. A non-positive wait means the throttling window has
        ...
        // Positive wait times are unaffected: they match the strict pattern above.
